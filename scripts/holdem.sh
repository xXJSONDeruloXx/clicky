#!/usr/bin/env bash
#
# scripts/holdem.sh — launch Texas Hold'em (bundle 33333) in headed mode.
#
# Usage:
#   ./scripts/holdem.sh                 # default: build + headed run
#   ./scripts/holdem.sh --timeout 15    # auto-terminate after 15 seconds
#   ./scripts/holdem.sh --headless      # no window (for CI / quick checks)
#   ./scripts/holdem.sh --verbose       # debug-level logging
#   ./scripts/holdem.sh --dump 30       # dump first 30 frames as PPM
#
...
GAME_NAME="holdem"
BUNDLE_ID="33333"
GAME_TITLE="Texas Hold'em"
DEFAULT_BUNDLE="${HOLDEM_BUNDLE:-$HOME/Downloads/16-ipod-games/Games_RO/$BUNDLE_ID}"
BUNDLE="$DEFAULT_BUNDLE"
LOG_DIR="${HOLDEM_LOG_DIR:-/tmp}"
CAPTURE_ROOT="${HOLDEM_CAPTURE_ROOT:-/tmp}"
...
if [[ ! -d "$BUNDLE" ]]; then
    ...
    echo "  set HOLDEM_BUNDLE or pass a path" >&2
    exit 1
fi
