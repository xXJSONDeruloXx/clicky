#!/usr/bin/env bash
#
# scripts/pacman.sh — launch Pac-Man (bundle AAAAA) in headed mode.
#
# Usage:
#   ./scripts/pacman.sh                 # default: build + headed run
#   ./scripts/pacman.sh --timeout 15    # auto-terminate after 15 seconds
#   ./scripts/pacman.sh --headless      # no window (for CI / quick checks)
#   ./scripts/pacman.sh --verbose       # debug-level logging
#   ./scripts/pacman.sh --dump 30       # dump first 30 frames as PPM
#
...
BUNDLE_ID="AAAAA"
GAME_TITLE="Pac-Man"
DEFAULT_BUNDLE="${PACMAN_BUNDLE:-$HOME/Downloads/16-ipod-games/Games_RO/$BUNDLE_ID}"
BUNDLE="$DEFAULT_BUNDLE"
LOG_DIR="${PACMAN_LOG_DIR:-/tmp}"
CAPTURE_ROOT="${PACMAN_CAPTURE_ROOT:-/tmp}"
...
if [[ ! -d "$BUNDLE" ]]; then
    ...
    echo "  set PACMAN_BUNDLE or pass a path" >&2
    exit 1
fi
