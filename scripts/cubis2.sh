#!/usr/bin/env bash
#
# scripts/cubis2.sh — launch Cubis 2 (bundle 99999) in headed mode.
#
# Usage:
#   ./scripts/cubis2.sh                 # default: build + headed run
#   ./scripts/cubis2.sh --timeout 15    # auto-terminate after 15 seconds
#   ./scripts/cubis2.sh --headless      # no window (for CI / quick checks)
#   ./scripts/cubis2.sh --verbose       # debug-level logging
#   ./scripts/cubis2.sh --dump 30       # dump first 30 frames as PPM
#
...
BUNDLE_ID="99999"
GAME_TITLE="Cubis 2"
DEFAULT_BUNDLE="${CUBIS2_BUNDLE:-$HOME/Downloads/16-ipod-games/Games_RO/$BUNDLE_ID}"
BUNDLE="$DEFAULT_BUNDLE"
LOG_DIR="${CUBIS2_LOG_DIR:-/tmp}"
CAPTURE_ROOT="${CUBIS2_CAPTURE_ROOT:-/tmp}"
...
if [[ ! -d "$BUNDLE" ]]; then
    ...
    echo "  set CUBIS2_BUNDLE or pass a path" >&2
    exit 1
fi
