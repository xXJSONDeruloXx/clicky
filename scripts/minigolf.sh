#!/usr/bin/env bash
#
# scripts/minigolf.sh — launch Mini Golf (bundle 88888) in headed mode.
#
# Usage:
#   ./scripts/minigolf.sh                 # default: build + headed run
#   ./scripts/minigolf.sh --timeout 15    # auto-terminate after 15 seconds
#   ./scripts/minigolf.sh --headless      # no window (for CI / quick checks)
#   ./scripts/minigolf.sh --verbose       # debug-level logging
#   ./scripts/minigolf.sh --dump 30       # dump first 30 frames as PPM
#
...
DEFAULT_BUNDLE="${MINIGOLF_BUNDLE:-$HOME/Downloads/16-ipod-games/Games_RO/$BUNDLE_ID}"
BUNDLE="$DEFAULT_BUNDLE"
LOG_DIR="${MINIGOLF_LOG_DIR:-/tmp}"
CAPTURE_ROOT="${MINIGOLF_CAPTURE_ROOT:-/tmp}"
...
if [[ ! -d "$BUNDLE" ]]; then
    ...
    echo "  set MINIGOLF_BUNDLE or pass a path" >&2
    exit 1
fi
