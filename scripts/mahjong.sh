#!/usr/bin/env bash
#
# scripts/mahjong.sh — launch Mahjong (bundle 77777) in headed mode.
#
# Usage:
#   ./scripts/mahjong.sh                 # default: build + headed run
#   ./scripts/mahjong.sh --timeout 15    # auto-terminate after 15 seconds
#   ./scripts/mahjong.sh --headless      # no window (for CI / quick checks)
#   ./scripts/mahjong.sh --verbose       # debug-level logging
#   ./scripts/mahjong.sh --dump 30       # dump first 30 frames as PPM
#
...
BUNDLE_ID="77777"
GAME_TITLE="Mahjong"
DEFAULT_BUNDLE="${MAHJONG_BUNDLE:-$HOME/Downloads/16-ipod-games/Games_RO/$BUNDLE_ID}"
BUNDLE="$DEFAULT_BUNDLE"
LOG_DIR="${MAHJONG_LOG_DIR:-/tmp}"
CAPTURE_ROOT="${MAHJONG_CAPTURE_ROOT:-/tmp}"
...
if [[ ! -d "$BUNDLE" ]]; then
    ...
    echo "  set MAHJONG_BUNDLE or pass a path" >&2
    exit 1
fi
