#!/usr/bin/env bash
#
# scripts/mspacman.sh — launch Ms. Pac-Man (bundle 14004) in headed mode.
#
# Usage:
#   ./scripts/mspacman.sh                 # default: build + headed run
#   ./scripts/mspacman.sh --timeout 15    # auto-terminate after 15 seconds
#   ./scripts/mspacman.sh --headless      # no window (for CI / quick checks)
#   ./scripts/mspacman.sh --verbose       # debug-level logging
#   ./scripts/mspacman.sh --dump 30       # dump first 30 frames as PPM
#
...
GAME_NAME="mspacman"
BUNDLE_ID="14004"
GAME_TITLE="Ms. Pac-Man"
DEFAULT_BUNDLE="${MSPACMAN_BUNDLE:-$HOME/Downloads/16-ipod-games/Games_RO/$BUNDLE_ID}"
BUNDLE="$DEFAULT_BUNDLE"
LOG_DIR="${MSPACMAN_LOG_DIR:-/tmp}"
CAPTURE_ROOT="${MSPACMAN_CAPTURE_ROOT:-/tmp}"
...
if [[ ! -d "$BUNDLE" ]]; then
    ...
    echo "  set MSPACMAN_BUNDLE or pass a path" >&2
    exit 1
fi
