#!/usr/bin/env bash
#
# scripts/simsbowling.sh — launch Sims Bowling (bundle 1500C) in headed mode.
#
# Usage:
#   ./scripts/simsbowling.sh                 # default: build + headed run
#   ./scripts/simsbowling.sh --timeout 15    # auto-terminate after 15 seconds
#   ./scripts/simsbowling.sh --headless      # no window (for CI / quick checks)
#   ./scripts/simsbowling.sh --verbose       # debug-level logging
#   ./scripts/simsbowling.sh --dump 30       # dump first 30 frames as PPM
#
# All CLICKY_* env vars can be overridden from your shell.

set -euo pipefail

GAME_NAME="simsbowling"
BUNDLE_ID="1500C"
GAME_TITLE="Sims Bowling"
DEFAULT_BUNDLE="${SIMSBOWLING_BUNDLE:-$HOME/Downloads/16-ipod-games/Games_RO/$BUNDLE_ID}"
BUNDLE="$DEFAULT_BUNDLE"
LOG_DIR="${SIMSBOWLING_LOG_DIR:-/tmp}"
CAPTURE_ROOT="${SIMSBOWLING_CAPTURE_ROOT:-/tmp}"
...
if [[ ! -d "$BUNDLE" ]]; then
    ...
    echo "  set SIMSBOWLING_BUNDLE or pass a path" >&2
    exit 1
fi
